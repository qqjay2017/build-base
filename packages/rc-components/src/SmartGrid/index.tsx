...
const SmartGridContainerStyle = styled.div<{
  ...
  @media (max-width: 1600px) {
    ...
    grid-template-rows: repeat( ${props=>Math.ceil(props.childNodeLength / Math.min(props.col,2))}, 1fr);
...
export function SmartGrid({ col=2, children,needOverHidden=false,...rest }: ISmartGridContainerProps) {
...
//   let latestIndex = 0;
  ...
  return (
    <SmartGridContainerStyle {...rest} col={col}  row={row} needOverHidden={needOverHidden} childNodeLength={childNodeLength} >
      {nodes}
