...
import { artListModel, getArtList, request } from "./utils/request";
...
export default function App({
  ...
  // useEffect(() => {
  //   request("/api/uims/v1/resource/getMenu/111875263834587161")
  //     .then((res:any) => {
  //       console.log(res)
  //       st(res);
  //     })
  ...
  const openDt = useCallback((path: string) => {
    if (path) {
      ...
      window.open(
        `${_href}/cms-static/${path}?busCode=cms1010&info=${Base64.encode(
  ...
  return (
    ...
    >
      <BannerLine>
        <BzzxText className="">帮助中心</BzzxText>
        <CloseImgWrap onClick={() => setOpen(false)}>
          <ArtRightArrow
            src={closeSrc}
            alt="close"
            width="16px"
          ...
          <div>
            {(artList || []).map((art, index) => {
              return (
                ...
                >
                  <ArtItemTitle>{art.title}</ArtItemTitle>
                  <ArtRightArrow alt="arrow" src={rightArrowSrc} />
                </ArtItem>
              );
            })}
          </div>
